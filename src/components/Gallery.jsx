...
const galleryItems = [
  ...
  // Swimwear - jacuzzi + studio bikini
...
export default function Gallery() {
  ...
  // For "Todos", show max 12 (3 from each category for a balanced grid)
  const displayed = activeFilter === 'all' ? filtered.slice(0, 12) : filtered;

  return (
    <section id="portfolio" className="py-28 md:py-40 bg-dark relative overflow-hidden noise">
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[700px] bg-gold/[0.015] rounded-full blur-[180px]" />

      <div className="max-w-7xl mx-auto px-6 lg:px-8 relative">
        {/* Header */}
        <div className="text-center mb-24 reveal">
          <p className="text-[10px] font-body font-medium tracking-mega uppercase text-gold/70 mb-5">Portfolio</p>
          ...
          {categories.map((cat) => {
            ...
            return (
              <button
                ...
                className={`px-6 py-2.5 text-[10px] font-body font-medium tracking-ultra uppercase transition-all duration-700 ${
                  isActive
                    ? 'glass-gold text-gold glow-gold'
                    : 'glass text-cream/30 hover:text-cream/60 hover:border-cream/10'
                }`}
              >
                {cat}
              </button>
        ...
        <div ref={scaleRef} className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 stagger">
          {displayed.map((item, i) => {
            ...
            return (
              <div
                key={item.id}
                className={`reveal-scale visible group relative overflow-hidden cursor-pointer ${
                  isFeatured ? 'sm:col-span-2 sm:row-span-2' : ''
                }`}
                onClick={() => setLightbox(item)}
              >
                <div className="aspect-square w-full bg-dark-200 border border-white/[0.03] relative overflow-hidden">
                  <img
                    src={item.src}
                    alt={item.label}
                  ...
                  {/* Hover: blur bg + glass overlay */}
                  <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-all duration-700">
                    <div className="absolute inset-0 bg-dark/50 backdrop-blur-sm" />
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <p className="font-display text-lg lg:text-xl italic text-cream mb-1">{item.label}</p>
                      <p className="text-gold/60 text-[9px] tracking-mega uppercase capitalize">{item.category}</p>
                      <div className="w-8 h-[1px] bg-gold/30 mt-4" />
                    </div>
                  </div>
                </div>
              </div>
      ...
      {lightbox && (
        ...
        >
          ...
          <button
            className="absolute top-6 right-8 z-10 text-cream/40 hover:text-gold text-2xl transition-colors duration-500"
            onClick={() => setLightbox(null)}
            aria-label="Cerrar"
          >
          ...
          >
            ...
            />
            <div className="absolute bottom-0 left-0 right-0 glass-dark p-4 text-center">
              <p className="font-display text-lg italic text-cream">{lightbox.label}</p>
              <p className="text-gold/50 text-[9px] tracking-mega uppercase capitalize">{lightbox.category}</p>
            </div>
          </div>
        </div>
      )}
